...
#
# 예전에 "가까운 행일수록 정확하다"고 적었던 측정 (y470 -0.10m ... y20 -0.54m) 은
# 틀렸다. 텔레포트 자세가 도로 방향과 어긋나 있어서, 멀리 볼수록 GT 쪽 횡방향이
# 커진 것을 인지 오차로 잘못 읽은 것이다.
#
# 도로 방향을 GT 에서 읽어 정확히 정렬시키고 다시 재면 (tools/lane_row_gt.py)
#   y470 -1.51m(±1.21)  y435 -0.09(±0.07)  y330 -0.08(±0.04)
#   y260 -0.08(±0.02)   y180 -0.09(±0.04)  y20  -0.10(±0.10)
# 즉 편향은 모든 행이 같고, 맨 아랫줄만 못 쓴다. 잡음이 가장 적은 곳은
# 6.3~7.8m 이지만, 주시거리를 그쪽으로 늘리면 (LD_MIN=5.6) diagnose 의 인지·제어
# 오차가 둘 다 커져 되돌렸다. 되먹임 거리는 아래 상수대로 둔다.
...
#
# 경로점은 두고 곡률에서만 y>=450 을 빼면 직진 편향은 -0.07 -> -0.01.
# 여기에 STEER_GAIN=1.40(≈1/0.71) 을 얹어도 실선 27%, |이탈| 0.44~0.48m.
# kappa_eval: 코너 추정곡률 ~100%인데도 실선이 남는다. 전역 조향 배수로는
# 부족분을 메우지 못한다. 동작은 기준선으로 되돌린다.
...
class MotionPlanningNode(Node):
    ...
    def update_bow(self):
        ...
        원인을 찾았다 (tools/lane_row_gt.py). 차를 GT 차로 중앙에 놓고 도로
        방향과 정렬시킨 뒤 행별로 재보니, 곧은 길에서 행 20~435 는 오차가
        -0.09m 로 거의 일정한데 맨 아랫줄 470 만 -1.51m ± 1.21m 였다. 카메라
        아래쪽은 보닛에 가려 차로 마스크가 그 전에 끝나므로, BEV 맨 아랫줄에는
        마스크의 잘린 아래 모서리만 걸린다 (그 행의 덩어리 폭이 72px 로, 정상인
        300px 의 4분의 1이다). 활 0.52m 의 거의 전부가 이 한 행에서 나온다.

        그런데 이것을 고쳐도 주행은 나빠진다. 이 가짜 곡률이 코너 조향을
        공급하고 있었기 때문이다 (tools/kappa_eval.py 로 GT 와 대조하면 코너에서
        필요량의 127%). 아래 세 가지를 재봤는데 모두 지금보다 나빴다.

            470 행 제외 + 조각 폭 하한 250px
                곡률은 정확해졌으나 (코너 102%) 급코너에서 쓸 수 있는 행이
                8개 -> 5개로 줄어 경로가 끊기고, 타성 주행으로 트랙을 벗어났다.
                조향 배수 1.45, 중심 오프셋 10px 로 보정해도 마찬가지였다.
            행간 일관성 검사로 튀는 행 버리기
                3랩은 완주했다. 곡률은 좋아졌지만 (직진 편향 -0.089 -> -0.051,
                코너 111%) 프레임마다 점 개수가 달라져 차가 더 흔들렸다
                (평균 이탈 0.27 -> 0.40m, 차선 밟음 9.8 -> 23.6%).
            튀는 행을 이웃 행 값으로 메우기
                흔들림이 조금 줄었다 (이탈 0.36m, 밟음 18.3%). 여전히 기준선보다
                나쁘다.
            메우기 + CENTER_OFFSET 25 + 원래 LD
                3랩 완주 1회 (밟음 21.1%, |이탈| 0.37m), 2회차는 정지.
                기준선(~10%, ~0.27m)보다 나쁘다.
            경로점은 유지하고 곡률 맞춤에서만 y>=450 제외
                직진 곡률 편향 -0.07 -> -0.01 로 좋아졌으나 실선 29~36%,
                |이탈| 0.48~0.54m. 코너 조향 명령이 필요량의 ~60%만 나갔다.
            위 제외 + STEER_GAIN=1.40
                실선 27%, |이탈| 0.44~0.48m. 전역 배수로는 안 된다.

        동작은 기준선으로 되돌린다. 다음 후보는 전역 게인이 아니라
        코너에서만 앞먹임(K_FF_DEFICIT)을 키우거나, 직진에서만 470을 빼는 식의
        구간 조건부 보정이다.
        """
        s = row_to_ahead(self.path_y)
        d = (self.path_x - CAR_CENTER_X) / PX_PER_M_LAT
